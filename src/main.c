...
int main(int argc, char **argv) {
  ...
  if ( strncmp( configuration.command, "helloworld", 10 ) == 0 ) {
    ...
    int center_x = atoi(configuration.filenames[0]);
    int center_y = atoi(configuration.filenames[1]);
    int width    = atoi(configuration.filenames[2]);
    int height   = atoi(configuration.filenames[3]);
